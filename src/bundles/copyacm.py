...
from src.utils import print_sub_step, log, execute
...
class CopyACM(Bundle):
    ...
    def configure(self, system_info, pre_launch_info, partitioning_info):
        if system_info["user_name"] != "":
            ...
            execute(f'mkdir -p /mnt/root/ArchCraftsman')
            execute(f'cp -r ~/src /mnt/root/ArchCraftsman')
            execute(f'cp -r ~/locales /mnt/root/ArchCraftsman')
